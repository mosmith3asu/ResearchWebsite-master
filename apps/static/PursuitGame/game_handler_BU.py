...
class GameHandler(object):
    ...
    def __init__(self,iworld,treatment,debug = False):
        ...
        self.move_buffer = {}
        self.move_buffer['move_R'] = None
        self.move_buffer['move_H'] = None
        self.move_buffer['move_E'] = None
    ...
    def new_move(self,t_post_move):
        ...
        if t_post_move >= total_delay:
            ...
            for key in self.move_buffer.keys():
                self.move_buffer[key] = None

    def execute_players(self):


        if  self.move_buffer['move_R']  is None:
            self.move_buffer['move_R'] = self.decide_robot_move()

        move_R = self.buffer['move_R']
    ...
    def decide_robot_move(self):
        ...
        for isoph in range(sophistocation):
            ...
            for k in range(n_agents):
                ...
                qAegok[k, :] = qAJ_conditioned @ ijoint
                pdAegok[k, :] = self.softmax_stable(rationality * qAegok[k, :])
                new_pdAjointk[k, :] = pdAegok[k,:] @ ijoint.T
