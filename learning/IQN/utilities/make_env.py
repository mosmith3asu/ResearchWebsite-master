...
class PursuitEvastionGame(Env):
    ...
    def step_prey(self):
        ...
        def prey_dist2q(dists,dmax):
            ...
            q_pow = 1. # exponential scaling on q val
            pref_closer = min(0.5, (dists.max() - dists.min()) / dmax)  # preferenc to avoid closer agent
            wmean_dists = (0.5 + pref_closer) * dists.min() + (0.5 - pref_closer) * dists.max()  # weighted dists
            q_res = q_scale * torch.pow(wmean_dists, q_pow)
            return q_res
